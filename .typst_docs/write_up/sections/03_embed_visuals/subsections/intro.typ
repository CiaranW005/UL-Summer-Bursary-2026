...
Embedding structure is examined qualitatively using PCA@pca, t-SNE@tsne, and UMAP@umap, and quantitatively using measures of cluster separation, compactness, and distance from the learned normal representation. These analyses are used to investigate differences between normal and defective samples, variation between dataset categories, and whether characteristics identified during the earlier EDA, such as defect coverage and visual structure, help explain the observed behaviour. The resulting embedding characteristics are also considered in relation to the baseline AUROC results to identify properties that may contribute to strong or weak anomaly separation.

Scoring metrics will be used to analyse cluster quality and separation, including:

- *Silhouette Score (Sil.)*@silhouette \
...
- *Davies-Bouldin Index (DB)*@davies_bouldin \
...
- *Calinski-Harabasz Index (CH)*@cal_har \
